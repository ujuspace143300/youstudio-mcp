...
- **결정이 필요하다**(셋 다 사용자 몫): ⓐ 그 자리에 나레를 더 쓴다(script 손댐 — 이번 지시에서 금지) · ⓑ 그 구간만큼 선택 구간을 줄인다(select 손댐 — 금지) · ⓒ 정답지 「자막.G-죽은시간_max」 대역을 재보정한다(측정이 정확해졌다는 근거로). 지금은 ⓒ 를 하지 않고 멈춰 있다 — 정답지는 목표라 사람이 정한다.
