...
| lb_transcript ②·lb_script ②·lb_blocks A (§94 SRT 정답 · 2026-09-04 사장님 승인 제안 2건) | `도구/srt후보읽기.py`(새 도구 — 드라마 폴더 *.srt 전부를 JSON 으로; 고르는 건 서버 `srt.ts pickSrt` = 키트 srt고르기.py 로직) · `도구/SRT블록.py`(사본에 `--구간시작/--구간끝` 추가 — EPnn 폴더는 이름에 구간이 없다) · `도구/자막경계맞춤.py` · `도구/대사빠짐검사.py`(볼트 승격판 — 경로 짐작 없음) · `도구/자막말머리맞춤.py`(볼트 승격판 — 재기 모드 ✗+rc1) | srt고르기 만 서버로 옮김 | 글자는 SRT·시각은 전사. 못 고르면 srt원본 을 안 쓴다(경고) — 틀린 SRT 를 무는 것이 가장 나쁘다 |
...
| lb_subs (한번에.sh ①·①.5) | `도구/서식.py`(신병4/서식.py — 곳간 신병4_본 값이 안에 있다) · `도구/폭맞춤.py` · `ass자리검사.py` · `구둣점검사.py` · `그래픽짓기.py` · `계획짓기.py` · `자막끝맞춤.py` · `원음스템.py` + `자막말머리맞춤.py`(§93, 서버사슬 한편.py 에서 여기로) | 같은 이름 | 로고판 ass 는 `grep -v credit_cta` 를 python -c 로. ★작품별 곳간 서식은 아직 없다 — 서식.py 표를 STYLE 로 빼는 것은 다음 과제 |
| lb_xml (한번에.sh ②) | `도구/xml짓기.py`(신병4/xml짓기.py 의 유스튜디오판 — `--작품 --자막 --배율` 인자, make_xml 은 같은 폴더에서) · `도구/make_xml.py` | 정본은 captions_신병4·106.25%·볼트 절대경로가 박혀 있었다 | 배율 = 1020/레터박스 뺀 그림 높이(lb_probe content_h) — 회귀 EP19 에서 정본 XML 과 uuid 빼고 같음 확인 |
| lb_prproj (한번에.sh ③~⑬.5) | `도구/프리미어돌리기.py`(새 — ③ 잠금·대기줄·CEP·_갓지은판, 맥·윈도우) · 키트 뿌리 그대로 `본떠서만들기.py` `서식입히기.py` `자리잡기.py` `꾸미기.py`(도구/ 를 형제로 찾는다) · `도구/` 소스텍스트·플랫버퍼·줄나누기·글꼴표·가로비율·그림자빼기·주입검사·자막자리검사·마스터효과심기·아모르입히기·제목끝맞춤 · `스타일/` 신병4_본.json·아모르_부품.prproj·**마스터효과_도너.prproj**(사장님 EP1 · 프리셋 카드 「prproj 오디오 마스터」) | 같은 이름 | 곳간은 «신병4» 고정(짝표) — 작품별 곳간은 다음 과제. `--확인만` 도구는 python -c 래퍼로 「확인만 rc=N」. CEP 확장(프리미어확장/)은 사용자가 설치 |
| lb_render (한번에.sh ⑭) | `도구/영상굽기.py`(신병4/영상굽기.py 의 유스튜디오판 — `--크롭 W:H:X:Y`, lb_probe 레터박스로 짓는다) · `마스터맞춤.py` · `라우드니스.py`(−14.0/TP −3.4) · `완성굽기.py` · `대조점짓기.py` · `_synccheck.py`(볼트 스크립트/) | 같은 이름 | 효과음은 편 폴더 효과음계획.json+add_sfx.py 가 있을 때만(python -c 분기). merged_균일 캐시 판단 없음 — 매번 굽는다 |
| lb_check (완성검사.sh 검사부) | python -c 검사 JSON(해상도·길이·ebur128·매트 좌우 40px 띠·효과 y·헤드라인·낭독·ass 지문 FNV-1a 64) · `글꼴폴백검사.py` · 마스터효과·제목끝맞춤 `--확인만` · 대사빠짐검사 · 자막말머리맞춤 | — | 완성검사 8 «서버 값» = lb_blocks C 가 carry 로 실은 ass_fp(FNV-1a 64) · 9·10·2 는 사람 확인 |
| lb_deliver (완성검사.sh 조립부) | python -c 조립 + `도구/납품SRT4벌.py` | — | human_ok 없이는 반려(검사 통과 ≠ 완성). NAS 납품은 사람 |
