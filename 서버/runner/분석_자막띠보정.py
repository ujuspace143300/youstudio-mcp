...
def 레인찾기(video, 매초=3.0, 잉크=200, 높이=540, 폭=160, 최소행비율=0.06, 최대행비율=0.6,
           레인빈도=0.15, 레인상한=0.85, 최대두께=0.25, 최소글자=2.5, H0=1080, limit_s=None):
    ...
    후보 = 구간([레인빈도 <= v <= 레인상한 for v in 빈도], 최소=3)
    자막레인, 기타 = [], []
    for s2, e2 in 후보:
        hs = []
        for g in 글자있음:
...
def main():
    ...
    행잉크 = ffmpeg_rows(a.video, a.매초, a.높이, a.폭, a.잉크, a.limit_s)
    if not 행잉크: raise SystemExit("프레임을 못 뽑았다")
    n = len(행잉크)
    최소칸 = max(1, int(a.폭 * a.최소행비율))
    최대칸 = int(a.폭 * a.최대행비율)
    # 「글자가 있다」 판정(2026-08-18 보정): 밝은 칸이 **너무 적지도 너무 많지도 않아야** 한다.
    #   가로가 거의 다 밝으면 글자가 아니라 **밝은 그림**이다(10편 「옥에 티」에서 그림 영역을 자막으로 착각했다).
    글자있음 = [[최소칸 <= f[r] <= 최대칸 for r in range(a.높이)] for f in 행잉크]
    빈도 = [sum(1 for g in 글자있음 if g[r]) / n for r in range(a.높이)]
    # 자막은 **떴다 사라진다**. 늘 켜져 있는 행은 헤더·워터마크·그림이다 → 위쪽 한계를 둔다.
    레인행 = [a.레인빈도 <= v <= a.레인상한 for v in 빈도]
    레인 = [[s, e] for s, e in 구간(레인행, 최소=3)]

    # 글자 높이: 레인 안에서 프레임마다 이어진 잉크 행 덩어리의 높이
    높이표 = []
    for s, e in 레인:
        hs = []
        for g in 글자있음:
            블록 = 구간([g[r] for r in range(s, e + 1)], 최소=1)
            if 블록: hs.append(max(b[1] - b[0] + 1 for b in 블록))
        hs.sort()
        중앙 = hs[len(hs) // 2] if hs else 0
        높이표.append({"레인_행": [s, e], "y_비율": [round(s / a.높이, 4), round((e + 1) / a.높이, 4)],
                      "글자높이_행_중앙": 중앙, "글자높이_px": round(중앙 * H0 / a.높이, 1),
                      "글자높이_화면대비_%": round(중앙 / a.높이 * 100, 2),
                      "글자_보인_프레임": len(hs), "표본_프레임": n})

    # 후보 가운데 **자막의 조건**을 갖춘 것만 레인으로 채택한다(2026-08-18 — 10편 「옥에 티」에서
    #   화면 위 고정 헤더(글자 0.2~1.1%)와 그림 영역(띠 두께 53%)이 섞여 들어왔다).
    #   조건: ① 띠 두께 ≤ 화면의 최대두께 ② 글자 높이 ≥ 최소글자. 나머지는 버리지 않고 「기타_요소」로 남긴다.
    자막레인, 기타 = [], []
    for x in 높이표:
        두께 = x["y_비율"][1] - x["y_비율"][0]
        (자막레인 if (두께 <= a.최대두께 and x["글자높이_화면대비_%"] >= a.최소글자) else 기타).append({**x, "띠_두께": round(두께, 4)})
