...
def 작표(words, slug, logline):
    ...
    try:
        ...
        # ★인물 이름 표기 (2026-09-07 사장님 «이름은 절대 오타내면 안 돼») — 낱말사전에서
        try:
            from s2pipe.asr import load_vocab as _lv
            _이름 = [e["content"] for e in _lv(slug)]
        except Exception:
            _이름 = []
        이름줄 = (f"- 인물 이름은 정확히 이 표기만 쓴다: {', '.join(_이름)}. 비슷하게 들려도"
                  f" 이 표기로 적되, **다른 인물에게 이 이름을 붙이지는 마라.**\n") if _이름 else ""
...
def main():
    ...
    try:
        ...
        if 분쟁:
            ...
            목록 = "\n".join(f"- t={d['t']:.1f}s A안(완성본 전사)「{d['text']}」 B안(원본 전사)「{w[:40]}」"
                             for d, w in 분쟁)
            try:
                from s2pipe.asr import load_vocab as _lv4
                _이름4 = [e["content"] for e in _lv4(proj["slug"])]
            except Exception:
                _이름4 = []
            pr = (f"숏폼 영상이다(로그라인: {proj.get('logline', '')}).\n"
                  + (f"인물 이름 표기(정확히): {', '.join(_이름4)} — 다른 인물에겐 붙이지 마라.\n"
                     if _이름4 else "")
    ...
    # ── 원문화면 구간 — 화면 속 글이 곧 자막이다 (2026-09-07 사장님 «결론 빼먹었어» —
    #    결말 명언 화면을 살리는 정식 통로). 그 구간 우리 대사 자막을 빼 두 겹을 막고,
    #    커버리지 게이트도 면제한다. 컷의 풀샷 유지는 준비 단계가 한다.
    _누 = 0.0
    for s in (x for x in proj.get("segments", []) if x.get("keep", True)):
        d0, d1 = _누, _누 + s["t1"] - s["t0"]
        _누 = d1
        if not s.get("원문화면"):
            continue
        뺌 = [d for d in dlg if d0 - 0.2 <= d["t"] < d1 + 0.2]
        for d in 뺌:
            dlg.remove(d)
        if 뺌:
            print(f"  원문화면 {d0:.1f}~{d1:.1f}s — 우리 대사 자막 {len(뺌)}줄 뺌(화면 글이 자막)")
        면제구간.append((d0 - 0.2, d1 + 0.2))
    ...
    # ★반복 의성어 축약 (2026-09-09 Deep54 «리리리리…» — 공백 없는 장문 괴성이 재분할 불가로
    #   관문을 죽였다. 같은 음절 반복 의성어는 표기 관행대로 짧게 줄인다 — 문구 발명이 아니라 축약.)
    for d in 관문:
        글 = d["text"]
        if len(글) > 최대 and " " not in 글 and len(set(글.replace("!", "").replace("~", ""))) <= 3:
            d["text"] = 글[:5] + "~"
            print(f"  ★반복 의성어 축약: 「{글[:18]}…」 → 「{d['text']}」")
    ...
    for a2, b2 in zip(dlg, dlg[1:]):
        if b2["t"] <= a2["t"] + 0.05:                       # 같은 시각 충돌
            ...
            for ln in (a2, b2):
                ...
                break
    겹침수, 병합수 = 0, 0
    for _바퀴 in range(3):     # 뒤 줄 밀기가 새 겹침을 만들 수 있어 최대 3바퀴 (2026-09-09 Deep57)
        dlg.sort(key=lambda x: (x["t"], x.get("t1", x["t"])))
        지울 = []
        고침 = 0
        for a2, b2 in zip(dlg, dlg[1:]):
            ...
            if b2["t"] - 0.03 - a2["t"] < 0.3 and len(a2["text"] + " " + b2["text"]) <= 14:
                ...
                # 바닥 0.15s — 0.3s 바닥이 0.1~0.2s 겹침을 못 풀었다(2026-09-07 «그러면서» 실측).
                # 다른 화자 트랙 간 겹침은 무해하지만 같은 트랙이면 조립이 죽으므로 여기서 푼다.
                a2["t1"] = round(max(a2["t"] + 0.15, b2["t"] - 0.03), 2)
                if a2["t1"] > b2["t"] - 0.03:
                    # ★시작이 사실상 같은 두 줄(합쳐 14자 초과) — 바닥으로 못 푼다: 뒤 줄을
                    #   앞 줄 끝 뒤로 민다 (2026-09-09 Deep57 «남들은 다|잘하는 거 같은데» 32.63s 동시)
                    b2dur = max(b2.get("t1", b2["t"]) - b2["t"], 0.5)
                    b2["t"] = round(a2["t1"] + 0.03, 2)
                    b2["t1"] = round(b2["t"] + b2dur, 2)
                    print(f"  ★동시 시작 줄 밀기: 「{b2['text'][:14]}」 → {b2['t']:.2f}s")
                겹침수 += 1
        for x in 지울:
            dlg.remove(x)
        if not 고침:
