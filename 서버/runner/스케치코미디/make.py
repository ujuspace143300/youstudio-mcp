...
def check(proj, path):
    ...
    try:
        ...
        for t in reversed(후보들[-4:]):          # 끝에서 최대 4개만 프레임을 재본다
            ...
            끝발화 = t
            break
        # ★반려가 아니라 «주의» (2026-09-07 사장님 «뒷부분 이어진 거 뭐야» — Deep10 에서
        #   이 게이트가 반려를 내는 바람에 원본 꼬리 개그를 억지로 붙였고 결말이 늘어졌다.
        #   원본의 마지막 발화 ≠ 이야기의 결 — 결은 최고점(펀치라인)에서 닫는 «판단»이다.
        #   기계는 꼬리 존재만 알리고, 담을지는 사람이 정한다. Deep09(진짜 결말 잘림)처럼
        #   의심되면 이 주의를 보고 확인한다.)
        if 끝발화 and segs[-1]["t1"] < 끝발화 - 1.0:
            warn.append(f"원본 발화가 {끝발화:.0f}초까지 이어지는데 마지막 조각은"
                        f" {segs[-1]['t1']:.0f}초에 끝난다 — 꼬리에 진짜 결말이 있는지"
                        f" 눈으로 확인하라 (있으면 담고, 아웃트로 개그면 두는 게 맞다)")
    except Exception:
        pass
