 ...
 *
 * 명령:
 *   node 도구/대장.js 발급  <이메일> [--일수 90] [--대수 2]     새 토큰 발급(랜덤) → 토큰 출력
...
if (cmd === "발급") {
  ...
  const lic = {
    ...
    maxDevices: dev, devices: [], blocked: false,
  };
  kvPut(token, lic);
  console.log("발급 완료 — 이 토큰을 지인에게 전달한다(한 번만 보인다):\n");
  console.log("  " + token + "\n");
  console.log(`  이메일 ${email} · 만료 ${lic.expires} · 기기 ${dev}대`);
} else if (cmd === "목록") {
  const out = kv(["key", "list"]);
  let keys = [];
  try { keys = JSON.parse(out); } catch { keys = []; }
  console.log("이메일\t만료\t기기\t대수\t차단");
  for (const k of keys) {
    const l = kvGet(k.name);
    if (l) console.log(`${l.email}\t${l.expires}\t${l.devices.length}\t${l.maxDevices}\t${l.blocked ? "★차단" : "-"}`);
  }
} else if (cmd === "보기") {
