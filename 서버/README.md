...
2. 프리미어 프로 실행 → **새 빈 프로젝트**(아무 이름). 같은 소재를 이미 넣어 둔 프로젝트에 다시 임포트하면 **오디오 트랙이 조용히 빠진다**(2026-08-16 실측, 설계/참고_export.md 8절) — 다시 넣을 때도 새 프로젝트.
...
6. **자막 위치 지정(트랙당 1회, 꼭 필요)**: 프리미어 2026 은 XML 의 자막 위치를 읽지 않고 왼쪽 위에 둔다(설계/참고_export.md 6-3). V3 트랙의 자막 클립을 전부 선택(첫 클립 클릭 → 마지막 클립 Shift+클릭) → 창 > Essential Graphics(기본 그래픽) > 편집 → 「정렬 및 변형」의 위치를 manifest.json `프리미어_후속` 의 값으로(나레 x·y, 가운데 정렬), 텍스트 폰트·크기가 다르면 같은 패널에서 고른다. V2 도 같은 방법. 값은 규격 「자막」(위치_center·크기_px·폰트)에서 온다.
