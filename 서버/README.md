...
6. 이상하면: 폰트가 다르게 보이면 자막 클립 선택 → Effect Controls > Text 에서 폰트를 직접 고른다(나레 Source Han Serif K Bold, 대사 Sandoll Gwanghwamun). XML 의 폰트 이름·세로 부호는 규격 「자막」(xml명·세로부호)에서 바꾼다 — 첫 임포트 실측은 설계/참고_export.md 5~6절. 나레가 안 들리면 A2 트랙 음소거를 확인. 원본 소리가 나레와 부딪히면 A3(덕킹 컷) 트랙을 내리거나 음소거한다.
