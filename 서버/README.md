...
**산출물은 프리미어 프로젝트 파일(.prproj) 이다. 가져오기(Import) 하지 않는다 — 그냥 연다.**

1. 산돌구름 앱이 켜져 있는지 확인한다(트레이 아이콘). 꺼져 있으면 폰트가 다른 서체로 나온다.
2. `youstudio_work/<영화>/render/<영화>.prproj` 를 **더블클릭**한다. 프리미어가 그 프로젝트를 연다.
3. 프로젝트 패널의 시퀀스("<제목> 리캡")를 더블클릭한다. V1 원본 컷 · V2 대사 자막 · V3 나레 자막 · A1 원본 소리(살릴 컷) · A2 나레이션 · **A3 덕킹 컷의 원본 소리**가 전부 제자리에 있다.
4. 재생만 하면 된다. 자막 위치·폰트·서식은 도너 견본 그대로라 **후속 작업이 없다**.
5. 미디어가 오프라인이면(원본 mp4·나레 wav 를 옮겼을 때) 프리미어가 물어보는 자리에서 새 위치를 한 번 잡아 준다(Link Media).
6. 자막을 SRT 로 쓰고 싶으면 `render/subtitle.srt`(합본) 또는 `_nar` / `_dlg` 를 파일 > 가져오기 → 캡션 트랙으로.

### 폴백 — FCP XML 로 열기

도너가 깨졌거나 프리미어 버전이 바뀌어 .prproj 가 안 열릴 때만. 규격 「조립.산출물」을 `"xml"` 로 바꾸고 export 를 다시 돌린 뒤:

1. 프리미어 → **새 빈 프로젝트**(아무 이름). 같은 소재를 이미 넣어 둔 프로젝트에 다시 임포트하면 **오디오 트랙이 조용히 빠진다**(2026-08-16 실측, 설계/진단일지.md 8절).
2. 파일 > 가져오기 → `render/<영화>.xml` → 시퀀스 하나가 생긴다.
3. **자막 위치 확인**: origin 파라미터로 나레 y≈840 · 대사 y≈980(1920×1080) 에 자동 배치된다(설계/진단일지.md 6-1b, 규격 「자막.위치」). 어긋나면 그 트랙 자막 클립을 전부 선택 → 창 > Essential Graphics > 편집 → 「정렬 및 변형」의 위치를 manifest.json `프리미어_후속` 값으로.
4. 폰트가 다르면 같은 패널에서 직접 고른다(나레 Source Han Serif K Bold, 대사 Sandoll Gwanghwamun) — XML 폰트 이름은 PS 명만 통한다.
