...
// 34) export ② — XML · SRT · manifest · 게이트 전체 재검사 → done
{
  ...
  ok(/<value>SDGwanghwamun<\/value>/.test(xml) && /<value>SourceHanSerifK-Bold<\/value>/.test(xml) && /<vert>300<\/vert>/.test(xml) && /<vert>440<\/vert>/.test(xml) && /file:\/\/localhost\/C:\/movies\/sample\.mp4/.test(xml), "export② → 폰트 xml명(PS: SDGwanghwamun·SourceHanSerifK-Bold) · 세로부호 +1(FCP 규약, vert 300/440) · pathurl", "");
  const man = sc?.write_files?.find((w) => /manifest\.json$/.test(w.path))?.content;
  ok(Array.isArray(man?.프리미어_후속) && man.프리미어_후속[0]?.위치_px?.y === 840 && man.프리미어_후속[1]?.위치_px?.y === 980 && man.프리미어_후속[0]?.위치_px?.x === 960, "export② → manifest.프리미어_후속(자막 위치 수동 지정값 V3 y=840 · V2 y=980, 규격 위치_center 에서 계산)", JSON.stringify(man?.프리미어_후속?.map((r) => r.위치_px)));
