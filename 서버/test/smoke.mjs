...
// 0) health
{
  ...
  // 관리 페이지(길 B) — dev 는 ADMIN_TOKEN 이 있으면(dev-smoke.sh 가 --var 로 준다) 인증까지, KV 는 없어 503
  const adm = await fetch(URL_ + "/admin");
  ...
  if (process.env.ADMIN_TOKEN) {
    ...
    ok(right.status === 503 && /LICENSES/.test((await right.json()).error ?? ""), "/admin/api/list 맞는 토큰(dev · KV 없음) → 503 «대장 없음»", String(right.status));
  }
...
// L6) lb_script — ① need_input ② 서버 검사 + 게이트 도구 ③ 로그 검사
{
  ...
  ok(fail.structuredContent?.status === "execute" && fail.structuredContent?.next_step === "lb_voice" && (fail.structuredContent?.warnings ?? []).some((w) => /b03/.test(w) && /손질/.test(w)), "lb_script③(대본검사 ✗) → 경고만 · lb_voice 로(관문은 lb_blocks 손질 뒤 — 정본 순서)", JSON.stringify(fail.structuredContent?.warnings).slice(0, 80));
...
// L13) lb_check — 완성검사 기계 항목 표
{
  ...
  const s2 = r2.structuredContent;
  ok(s2?.status === "execute" && s2?.next_step === "lb_deliver" && s2?.metrics?.passed === 13 && s2?.metrics?.pending === 0 && (s2?.check_table ?? []).length === 13 && (s2?.instructions ?? []).some((x) => /사람 확인 9\./.test(x)) && (s2?.instructions ?? []).some((x) => /낭독 전문/.test(x)), "lb_check② → 기계 항목 13 통과(길이 51.6초는 규격 40~60 안) · 사람 확인 9·10 + 낭독 전문 → lb_deliver", JSON.stringify(s2?.metrics));
