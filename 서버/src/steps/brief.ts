  ...
  run({ preset, payload }) {
    ...
    return base("brief", preset, {
      ...
      instructions: [
        ...
        `④ select 는 세 가지를 더 받는다 — payload.brief: 방금 쓴 ${outPath} 의 내용(사건 목록, 작다) · payload.facts: <workdir>/facts.json 의 내용(사람이 잰 소재 사실 — 크레딧 시작·무음 구간. 없으면 {}) · payload.utterance_spans: transcript.json 의 utterances 에서 [start,end] 쌍만 뽑은 배열(본문 없이 타임코드만).`,
      ],
      then_call_with: ["step: 'select'", "payload: { workdir, source, probe_summary, transcript_path, brief_path, brief: <brief.json 내용>, facts: <facts.json 내용 또는 {}>, utterance_spans: [[start,end],…] }"],
      jobs_kind: null,
      jobs: [],
